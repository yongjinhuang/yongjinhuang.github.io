...
A comprehensive guide to titles, lower thirds, transitions, text animations, and visual
effects for video content. This is where you make your videos look polished and
professional — without needing a film studio.

---

## Table of Contents

1. [What Are Motion Graphics?](#1-what-are-motion-graphics)
2. [Essential Motion Graphics for YouTube](#2-essential-motion-graphics-for-youtube)
3. [Typography for Video](#3-typography-for-video)
4. [Title Design](#4-title-design)
5. [Lower Thirds](#5-lower-thirds)
6. [Text Animations and Callouts](#6-text-animations-and-callouts)
7. [Transitions and Effects](#7-transitions-and-effects)
8. [Screen Recording Overlays](#8-screen-recording-overlays)
9. [Tools for Motion Graphics](#9-tools-for-motion-graphics)
10. [Building a Motion Graphics Library](#10-building-a-motion-graphics-library)

---

## 1. What Are Motion Graphics?

Motion graphics are animated visual elements that communicate information, add style,
or guide viewer attention. Unlike VFX (which alters live footage), motion graphics are
created from scratch — text, shapes, icons, and illustrations brought to life.

```
Static graphics:          Motion graphics:
┌──────────────┐          ┌──────────────┐
│              │          │   ╱──────    │
│  TITLE TEXT  │   ──►    │  TITLE TEXT  │  ← slides in, bounces, fades
│              │          │    ──────╲   │
└──────────────┘          └──────────────┘
   (boring)                  (engaging)
```
...
│  │     "5 Tips for..."         │    (first 3-5 seconds)    │
│  └─────────────────────────────┘                            │
│                                                             │
│  ┌──────────────────┐                                       │
│  │ 👤 Speaker Name   │ ← 2. Lower third                    │
│  │    @handle        │    (identifies who is talking)       │
│  └──────────────────┘                                       │
│                                                             │
│           ┌────────────────┐                                │
│           │  Key Point #1  │ ← 3. Text callout             │
│           │  appears here  │    (emphasizes spoken words)   │
│           └────────────────┘                                │
│                                                             │
│                              ┌──────────────────────────┐   │
│                              │  Subscribe 🔔            │   │
...
┌────────────────────────────────────────┐
│  1. SANS-SERIF FOR SCREEN (PREFERRED)  │
│     Montserrat, Inter, Roboto,         │
│     Poppins, Oswald, Bebas Neue        │
│                                        │
│  2. SERIF FOR ELEGANCE                 │
...
### 4.1 Title Card Styles

```
Style 1: Full Screen Title
┌──────────────────────────────────────┐
│                                      │
│                                      │
│         HOW I LEARNED                │
...
Style 2: Lower Third Title (over footage)
┌──────────────────────────────────────┐
│         (footage behind)             │
│                                      │
│                                      │
...
Style 3: Animated Text (words appear one by one)
┌──────────────────────────────────────┐
│                                      │
│         HOW ─────────────────        │
│               I ─────────────        │
...
│                                                          │
│  ┌──────────────────────────────┐                        │
│  │ ■ John Smith                 │ ← Primary text (name)  │
│  │   Software Engineer @Google  │ ← Secondary text (role)│
│  └──────────────────────────────┘                        │
│     ↑                                                    │
│     Lower third zone (bottom 1/3 of screen)              │
└──────────────────────────────────────────────────────────┘
```

### 5.2 Lower Third Styles
...
```
Style 1: Pop-up beside speaker          Style 2: Full-screen emphasis
┌──────────────────────────┐            ┌──────────────────────────┐
│                          │            │                          │
│  👤  ┌───────────┐       │            │    ┌──────────────┐      │
│      │ KEY POINT │       │            │    │              │      │
...
Style 3: Handwritten/arrow              Style 4: Progress tracker
┌──────────────────────────┐            ┌──────────────────────────┐
│                          │            │  Step 1 ✓                │
│  👤  ←── THIS! ✏️        │            │  Step 2 ✓                │
│          (doodle style)  │            │  ▶ Step 3 (current)      │
│                          │            │  Step 4                  │
│                          │            │  Step 5                  │
└──────────────────────────┘            └──────────────────────────┘
```

### 6.3 Auto-Captions as a Design Element

Modern YouTube videos use large, animated captions as a creative element (not just
accessibility). This style was popularized by creators like MrBeast and Alex Hormozi.

```
Traditional subtitles:                  Modern animated captions:
┌──────────────────────────┐           ┌──────────────────────────┐
│                          │           │                          │
│       👤                 │           │       👤                 │
│                          │           │                          │
│                          │           │    THIS is the           │
│ This is the most         │           │    MOST IMPORTANT        │
│ important thing          │           │    thing                 │
└──────────────────────────┘           └──────────────────────────┘
  Small, bottom, static                  Large, centered, word-by-word
                                         with color emphasis on key words
```
...
- **DaVinci Resolve Subtitle** — Manual but precise
...
A powerful technique that changes playback speed within a single clip for dramatic effect.

```
Speed:   100%   100%   ▼ 25%   25%   25%   ▲ 100%   100%
         │       │      │      │     │      │        │
Clip:    ████████████▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓████████████████
              ↑            ↑              ↑
         Normal       Slow motion      Back to normal
         speed      (dramatic moment)    speed

Common use: Action moment slows down, then speeds back up
Requires: Footage shot at 60fps+ for smooth slow-motion
```

## 8. Screen Recording Overlays

### 8.1 For Tech/Tutorial Content

If you make tech tutorials or coding content, these overlays are essential:
...
│                                          └──────┘│
│  ┌──────────────────────────────────┐            │
│  │ Current Topic: Setting up Docker │            │
│  └──────────────────────────────────┘            │
└──────────────────────────────────────────────────┘
```

### 8.2 Zoom and Highlight Techniques

```
Technique 1: Zoom to code                Technique 2: Highlight box
┌─────────────────────┐                  ┌─────────────────────┐
│                     │                  │  code code code     │
│   ┌─────────┐       │                  │  code code code     │
│   │ const x │       │                  │ ┌─────────────────┐ │
│   │ = await │ ← zoom│                  │ │ highlighted line│ │
│   │ fetch() │   in  │                  │ └─────────────────┘ │
│   └─────────┘       │                  │  code code code     │
│                     │                  │  code code code     │
└─────────────────────┘                  └─────────────────────┘
...
```
Beginner:
  Start with CapCut or Canva for basic text and animations
  Learn DaVinci Resolve's built-in title tools
       │
       ▼
Intermediate:
  Learn DaVinci Resolve Fusion for custom animations
  OR learn After Effects basics
       │
       ▼
Advanced:
  Master After Effects + expressions (JavaScript-like scripting)
  Create reusable templates (MOGRTs for Premiere, Fusion macros for Resolve)
       │
       ▼
Expert:
  Build complex composites
  Create and sell your own template packs
  Combine 3D elements (Blender) with motion graphics
```
...
### 10.1 Create Once, Use Forever

Build a personal library of reusable elements:

```
📁 Motion_Graphics_Library/
├── 📁 Intros/
│   ├── Channel_Intro_v3.drp          (DaVinci Resolve project)
│   └── Channel_Intro_v3.mp4          (pre-rendered for quick use)
├── 📁 Lower_Thirds/
│   ├── Standard_LowerThird.drp
│   └── Guest_LowerThird.drp
├── 📁 Transitions/
│   ├── Whoosh_Transition.mp4         (pre-rendered)
...
```
New video:
1. Open project template (pre-configured timeline with placeholders)
2. Drop in footage
3. Drag in lower third from library → change text
4. Drag in title card from library → change text
5. Drag in end screen from library → update video links
6. Total graphics time: 10-15 minutes (vs 1-2 hours from scratch)
```
...
**Previous**: [05 - Color Grading & Correction](05-COLOR-GRADING-CORRECTION.md)
**Next**: [07 - Thumbnails & Branding](07-THUMBNAILS-BRANDING.md)
